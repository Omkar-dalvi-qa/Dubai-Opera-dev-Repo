...
/**
 * Same-origin proxy for `POST /events/reservation/transaction`.
 * Browser calls this route (no CORS); server forwards with `X-Access-Key-Secret`.
 */
export async function POST(req: NextRequest) {
  ...
  const result = await eventService.post<{
    ...
    message?: string;
    data?: unknown;
  }>("/external/booking/checkout", { body });
