pipeline {
    ...
    stages {
        ...
        stage('Read Dev Commit') {
            steps {
                script {
                    env.DEV_AUTHOR = sh(script: "git log -1 --pretty='%an'", returnStdout: true).trim()
                    env.DEV_EMAIL  = sh(script: "git log -1 --pretty='%ae'", returnStdout: true).trim()
                    env.DEV_MSG    = sh(script: "git log -1 --pretty='%s'",  returnStdout: true).trim()
                    env.DEV_DATE   = sh(script: "git log -1 --pretty='%ad' --date=format:'%d %b %Y %H:%M'", returnStdout: true).trim()
                    env.DEV_HASH   = sh(script: "git log -1 --pretty='%h'",  returnStdout: true).trim()
        ...
        stage('Parse Results') {
            steps {
                script {
                    ...
                    dir('test-suite') {
                        env.TEST_AUTHOR = sh(script: "git log -1 --pretty='%an'", returnStdout: true).trim()
                        env.TEST_MSG    = sh(script: "git log -1 --pretty='%s'",  returnStdout: true).trim()
                        env.TEST_DATE   = sh(script: "git log -1 --pretty='%ad' --date=format:'%d %b %Y %H:%M'", returnStdout: true).trim()
                    }
                ...
                <table width="100%" style="background:#333;padding:12px 32px;">
                    <tr><td>
                        <p style="margin:0;color:#888;font-size:11px;">
                            Jenkins CI — Dubai Opera Test Pipeline — Auto triggers on developer push
                        </p>
                    </td></tr>
                </table>
